...
> Propuesta de diseño. **No implementada todavía** — a discutir antes de escribir código.
> Contexto de evaluación: simulación del ecosistema de Mercado Libre (250+ equipos,
> escala de millones de requests), aunque el prototipo corra a escala mínima.

## 0. Alcance y decisiones ya acordadas
...
├── docs/
│   ├── Data Governance JD Positions.pdf   # enunciado original
│   └── architecture.md                    # este documento
├── notebooks/
│   └── meli_enrichment_pipeline.ipynb     # sin tocar
├── src/
│   └── meli_api/
...
│       │   ├── inbound/
│       │   │   └── http/
│       │   │       ├── app.py             #   FastAPI app factory, wiring de dependencias
│       │   │       ├── routers/
│       │   │       │   ├── products.py
│       │   │       │   └── health.py
...
│
├── tests/
│   ├── unit/            # domain + use cases, puertos mockeados, sin I/O
│   ├── integration/     # sqlite_repository y redis_cache contra instancias reales/temp
│   └── e2e/             # FastAPI TestClient, stack completo
│
...
## 2. Puertos (interfaces) — contratos que cruzan la frontera hexagonal
...
## 3. Resiliencia y degradación graceful (dentro de la API)
...
## 4. Cache con Redis — qué se cachea y cómo
...
- **TTL:** a definir (propongo 5 min para detalle, 1 min para listados — los listados
  cambian más si el pipeline corre de nuevo). **A confirmar con vos.**
...
## 6. Diseño pensado para escala (documentado, no todo implementado)
...
4. **Estructura de carpetas y capas**: confirmada tal como está descripta en este
   documento (secciones 1 y 2).

## 8bis. Nota de implementación: latencia real de Redis caído
...
1. **`redis-py` reintenta por su cuenta.** Desde la v5, el cliente trae por default
   ...
   enteramente al circuit breaker (sección 3), e inflaba la latencia de una sola
...
### 10.1 Diagrama de componentes -- capas y dirección de dependencia

Versión diagramada del ASCII de la sección 1: qué depende de qué, y por qué un cambio
...
### 10.2 Diagrama de secuencia -- `GET /products/{item_id}`
...
### 10.3 Diagrama de clases -- dominio y puertos
